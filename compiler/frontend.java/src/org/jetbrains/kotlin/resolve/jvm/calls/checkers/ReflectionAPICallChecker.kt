...
class ReflectionAPICallChecker(private val module: ModuleDescriptor) : CallChecker {
    private val isReflectionAvailable by lazy { module.findClassAcrossModuleDependencies(JvmAbi.REFLECTION_FACTORY_IMPL) != null }
    ...
    override fun <F : CallableDescriptor> check(resolvedCall: ResolvedCall<F>, context: BasicCallResolutionContext) {
        ...
        val containingPackage = DescriptorUtils.getParentOfType(containingClass, PackageFragmentDescriptor::class.java) ?: return
        if (containingPackage.fqName != KOTLIN_REFLECT_FQ_NAME) return
        ...
        if (name.asString() == "name") return

        if (name == OperatorConventions.INVOKE) return

        if ((name.asString() == "get" || name.asString() == "set") &&
