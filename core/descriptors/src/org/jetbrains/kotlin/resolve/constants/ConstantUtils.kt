...
public fun createCompileTimeConstant(
    ...
    return when(value) {
        ...
        is Float -> FloatValue(value, parameters.canBeUsedInAnnotation, parameters.usesVariableAsConstant)
        is Double -> DoubleValue(value, parameters.canBeUsedInAnnotation, parameters.usesVariableAsConstant)
        is Boolean -> BooleanValue(value, parameters.canBeUsedInAnnotation, parameters.usesVariableAsConstant)
        is String -> StringValue(value, parameters.canBeUsedInAnnotation, parameters.usesVariableAsConstant)
        null -> NullValue
        else -> null
    }
}
...
private fun getIntegerValue(
    ...
    if (TypeUtils.noExpectedType(expectedType) || expectedType.isError()) {
        return IntegerValueTypeConstant(value, parameters.canBeUsedInAnnotation, parameters.usesVariableAsConstant)
    }
